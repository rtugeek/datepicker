...
public class DPCNCalendar extends DPCalendar {
    ...
    private static final int[][] FESTIVAL_GREGORIAN_DATE = {
            {1},
            {14},
            {8, 12, 14, 15},
            {1, 5, 22},
    ...
    private class SimpleGregorian {
        int d;
        int m;
        int y;
    }

    private class SimpleLunar {
        int d;
        int m;
        int y;
    ...
    public String[][] buildMonthLunar(int year, int month) {
        String[][] gregorianMonth = buildMonthGregorian(year, month);
        SimpleGregorian simpleGregorian = new SimpleGregorian();
        String tmp[][] = new String[6][7];
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[0].length; j++) {
                tmp[i][j] = "";
                if (!TextUtils.isEmpty(gregorianMonth[i][j])) {
                    simpleGregorian.y = year;
                    simpleGregorian.m = month;
                    simpleGregorian.d = Integer.valueOf(gregorianMonth[i][j]);
                    SimpleLunar simpleLunar = null;
                    if (year >= 1900 && year <= 2100) {
                        simpleLunar = GregorianToLunar(simpleGregorian);
                    }
                    char[] c = String.valueOf(simpleLunar.d).toCharArray();
                    tmp[i][j] = lNumToStr(c);
                }
            }
        }
    ...
    private String[][] buildFestival(int year, int month) {
        String[][] gregorianMonth = buildMonthGregorian(year, month);
        SimpleGregorian simpleGregorian = new SimpleGregorian();
        String tmp[][] = new String[6][7];
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[0].length; j++) {
                ...
                if (!TextUtils.isEmpty(gregorianMonth[i][j])) {
                    ...
                    if (year >= 1900 && year <= 2100) {
                        simpleLunar = GregorianToLunar(simpleGregorian);
                        result = getFestivalLunar(simpleLunar.m, simpleLunar.d);
                    }
                    if (TextUtils.isEmpty(result)) {
                        result = getFestivalGregorian(simpleGregorian.m, simpleGregorian.d);
                        if (!TextUtils.isEmpty(result)) {
                            tmp[i][j] = result + "F";
                        }
                    } else {
    ...
    private String getFestivalLunar(int month, int day) {
        String tmp = "";
    ...
    private String getFestivalGregorian(int month, int day) {
        String tmp = "";
        int[] daysInMonth = FESTIVAL_GREGORIAN_DATE[month - 1];
        for (int i = 0; i < daysInMonth.length; i++) {
            if (day == daysInMonth[i]) {
                tmp = FESTIVAL_GREGORIAN[month - 1][i];
            }
        }
        return tmp;
    }

    private SimpleLunar GregorianToLunar(SimpleGregorian simpleGregorian) {
        int index = simpleGregorian.y - DAYS_AND_LEAP_MONTH_OF_LUNAR[0];
        int data = (simpleGregorian.y << 9) | (simpleGregorian.m << 5) | (simpleGregorian.d);
        ...
        long offset = GregorianToNum(simpleGregorian.y, simpleGregorian.m, simpleGregorian.d) - GregorianToNum(y, m, d);
        ...
        lunarD = (int) (offset);
        SimpleLunar simpleLunar = new SimpleLunar();
        simpleLunar.y = lunarY;
        simpleLunar.m = lunarM;
        simpleLunar.isLeap = false;
        if (leap != 0 && lunarM > leap) {
            simpleLunar.m = lunarM - 1;
            if (lunarM == leap + 1) {
                simpleLunar.isLeap = true;
            }
        }
        simpleLunar.d = lunarD;
        return simpleLunar;
    }
