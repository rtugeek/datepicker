...
import com.rtugeek.datepicker.bizs.calendars.DPCManager;
...
public class MonthView extends View {
    ...
    private int centerYear, centerMonth, selectedDayOfMonth;
    ...
    private float offsetYSubscript1, offsetYSubscript2;
    ...
    public MonthView(Context context) {
        ...
        selectedDayOfMonth = DateTime.now().getDayOfMonth();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                    ...
                    if (Math.abs(lastPointY - event.getY()) > 50) {
                        if (lastPointY < event.getY()) {
                            if (Math.abs(lastPointY - event.getY()) >= criticalHeight) {
                                ...
                                if (centerYear < 1901) centerYear = 1901;
                            }
                        } else if (lastPointY > event.getY()) {
                            if (Math.abs(lastPointY - event.getY()) >= criticalHeight) {
                                indexYear++;
                                centerYear = centerYear + 1;
                                if (centerYear > 2099) centerYear = 2099;
                            }
                        }
                ...
                } else if (mSlideMode == SlideMode.HORIZONTAL) {
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH) {
        ...
        offsetYSubscript1 = (((Math.abs(mPaint.ascent() + mPaint.descent())) / 2F) +
                heightFestival / 2F + heightGregorian / 2F) / 2F;
        offsetYSubscript2 = offsetYSubscript1 * 2F;
    ...
    private void draw(Canvas canvas, int x, int y, int year, int month) {
        ...
        DPInfo[][] result;
        Region[][] tmp;
        if (TextUtils.isEmpty(info[4][0].strGregorian)) {
            tmp = MONTH_REGIONS_4;
            arrayClear(INFO_4);
            result = arrayCopy(info, INFO_4);
        } else if (TextUtils.isEmpty(info[5][0].strGregorian)) {
            tmp = MONTH_REGIONS_5;
            arrayClear(INFO_5);
    ...
    private void draw(Canvas canvas, Rect rect, DPInfo info) {
        ...
        drawDate(canvas, rect, info.strGregorian, info.isWeekend);
        if (isFestivalDisplay) {
            if (info.isFestival) {
                drawFestival(canvas, rect, info.strFestival);
            } else {
                drawSubscript(canvas, rect, info.strLunar);
            }
        }
    ...
    private void drawBG(Canvas canvas, Rect rect, DPInfo info) {
        if (null != mDPDecor && info.isDecorBG) {
            mDPDecor.drawDecorBG(canvas, rect, mPaint,
                    centerYear + "-" + centerMonth + "-" + info.strGregorian);
        }
        ...
        if (info.strGregorian.equals(String.valueOf(selectedDayOfMonth))) {
            drawBGSelectedDay(canvas, rect);
        }
    }

    private void drawBGSelectedDay(Canvas canvas, Rect rect) {
        mPaint.setColor(mTManager.colorBGCircle());
    ...
    private void drawDate(Canvas canvas, Rect rect, String str, boolean isWeekend) {
        mPaint.setTextSize(sizeTextGregorian);
        if (isWeekend) {
        ...
        if (str.contains("&")) {
            ...
            if (mPaint.measureText(str1) > rect.width()) {
                ...
                canvas.drawText(str1.substring(0, length), rect.centerX(),
                        rect.centerY() + offsetYSubscript1, mPaint);
                canvas.drawText(str1.substring(length), rect.centerX(),
                        rect.centerY() + offsetYSubscript2, mPaint);
            } else {
    ...
    private void drawSubscript(Canvas canvas, Rect rect, String str) {
    ...
    private void drawDecor(Canvas canvas, Rect rect, DPInfo info) {
        if (!TextUtils.isEmpty(info.strGregorian)) {
            String data = centerYear + "-" + centerMonth + "-" + info.strGregorian;
    ...
    private void defineRegion(int x, int y) {
        ...
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (TextUtils.isEmpty(mCManager.obtainDPInfo(centerYear, centerMonth)[i][j].strGregorian)) {
                    continue;
                }
                if (region.contains(x, y)) {
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        ...
                        final String date = centerYear + "-" + centerMonth + "-" +
                                mCManager.obtainDPInfo(centerYear, centerMonth)[i][j].strGregorian;
                        ...
                        if (null != onDateChangeListener) {
                            selectedDayOfMonth = Integer.parseInt(mCManager.obtainDPInfo(centerYear,
                                    centerMonth)[i][j].strGregorian);
                            onDateChangeListener.onDateChange(selectedDayOfMonth);
                        }
    ...
    private void computeDate() {
        ...
        if (null != onDateChangeListener) {
            ...
            onDateChangeListener.onMonthChange(getCurrentMonth());
        }
    }
