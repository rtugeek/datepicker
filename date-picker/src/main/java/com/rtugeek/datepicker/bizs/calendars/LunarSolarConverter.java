...
public class LunarSolarConverter {
	...
	public static DateTime LunarToSolarDateTime(Lunar lunar){
		Solar solar = LunarToSolar(lunar);
		return solar.toDateTime();
	}
	...
	public static Lunar SolarToLunar(Solar solar) {
		...
		int solarYear = GetBitInt(solar11, 12, 9);
		int solarMonth = GetBitInt(solar11, 4, 5);
		int solarDay = GetBitInt(solar11, 5, 0);
		long offset = SolarToInt(solar.year, solar.month,
				solar.dayOfMonth) - SolarToInt(solarYear, solarMonth, solarDay);
		...
		if (leap != 0 && lunarMonth > leap) {
			lunar.month = lunarMonth - 1;
			if (lunarMonth == leap + 1) {
				lunar.isLeap = true;
			}
		}

		lunar.dayOfMonth = lunarDay;
		return lunar;
	}


	public void isLunarLeapYear(int year){
		int days = lunar_month_days[year - lunar_month_days[0]];
		int leap = GetBitInt(days, 4, 13);
	}

	public static Date SolarToLunar(Date date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTimeInMillis(date.getTime());
		Solar solar = new Solar();
		solar.setDayOfMonth(calendar.get(Calendar.DAY_OF_MONTH));
		solar.setMonth(calendar.get(Calendar.MONTH) + 1);
		solar.setYear(calendar.get(Calendar.YEAR));
		Lunar lunar = SolarToLunar(solar);
		return lunar.toDate();
	}

	public static DateTime LunarToSolar(DateTime date) {
		Lunar lunar = new Lunar();
		lunar.setDayOfMonth(date.getDayOfMonth());
		lunar.setMonth(date.getMonthOfYear());
		lunar.setYear(date.getYear());
		Solar solar = LunarToSolar(lunar);
		return solar.toDateTime();
	}

	public static Lunar SolarToLunar(DateTime date) {
		Solar solar = new Solar();
		solar.setYear(date.getYear());
		solar.setMonth(date.getMonthOfYear());
		solar.setDayOfMonth(date.getDayOfMonth());
		Lunar lunar = SolarToLunar(solar);
		return lunar;
	}

	@Deprecated
	public static Date LunarToSolar(Date date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTimeInMillis(date.getTime());
		Lunar lunar = new Lunar();
		lunar.setDayOfMonth(calendar.get(Calendar.DAY_OF_MONTH));
		lunar.setMonth(calendar.get(Calendar.MONTH) + 1);
		lunar.setYear(calendar.get(Calendar.YEAR));
		Solar solar = LunarToSolar(lunar);
		return solar.toDate();
	}
